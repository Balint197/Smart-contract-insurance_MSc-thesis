...
describe("Insurance", function () {
  ...
  it("Should make sure withdraws work", async function () {
    ...
    const createInsuranceTx2 = await insurance.connect(owner).createInsuranceContract(3, 15, 1, "Test contract", { value: ethers.utils.parseEther("1.0") });
    await createInsuranceTx2.wait();

    expect(await insurance.owner(0)).to.equal(owner.address);

    // checking deposits
    expect(await insurance.connect(owner).addressDeposits(0, owner.address)).to.equal(ethers.utils.parseEther("1.0"));
    expect(await insurance.connect(addr1).addressDeposits(0, addr1.address)).to.equal(ethers.utils.parseEther("0.0"));

    expect(await insurance.connect(owner).addressMaxWithdraw(0, owner.address)).to.equal(ethers.utils.parseEther("1.0"));
    expect(await insurance.connect(addr1).addressMaxWithdraw(0, addr1.address)).to.equal(ethers.utils.parseEther("0.0"));

    // withdraw 0.4 from owner (0.6 left)
    const validWithdrawTx = await insurance.connect(owner).withdraw(0, ethers.utils.parseEther("0.4"));
    await validWithdrawTx.wait();
    expect(await insurance.addressDeposits(0, owner.address)).to.equal(ethers.utils.parseEther("0.6"));

    // try to withdraw 0.4 from add1 (has no deposits)
